...
module.exports = library.export(
  ...
  function(element) {
    ...
    function renderCode(bridge, lines) {
      ...
      var linesOfHtml = lines.map(function(line) {
        ...
        var spaces = line.match(
          ...
          0].
          length
        var width = spaces/1.5+
          "em"
        line = line.slice(
          spaces)
        var html = "<line style=\"padding-left: "+
          width+
          "\">"
        var sym
        var txt
        ...
        function moreToBite() {
          var sym = grabSymbol(line)
          if (sym){
            line = line.slice(
              sym.length)
            return true }
          var txt = grabText(line)
          if (txt){
            line = line.slice(
              txt.length)
            return true }
    ...
    function grabText(line) {
      if (grabSymbol(
        line)){
        return}
      if (line.length > 0) {
        var last3 = line.substr(
          line.length - 3,
          3)
        if (last3 == "new") {
          var withoutNew = line.substr(
            0,
            line.length - 3)
          var text = upTilSymbols(withoutNew)
        } else {
          var text = upTilSymbols(
            line)}
        return text}}
